...
Сценарий:
1. Сохраните до **5** референс-наборов (в каждом до **8** фото) и задайте название
2. «Новая генерация» → без референсов / выбрать один сохранённый набор / загрузить новый
3. Бот повторно показывает выбранные фото → промпт → качество → формат → результат
